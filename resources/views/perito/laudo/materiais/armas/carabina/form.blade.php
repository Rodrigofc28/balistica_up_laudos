...
@if ($acao == 'Cadastrar')
{!! Form::open(['route' => ['carabinas.store', $laudo ], 'files' => true]) !!}
...
<div class="col-lg-12" style="padding: 0 5% 0">
    <div class="row mb-3">
        @foreach($array_gdl_armas as $carabina_gdl)
            <span hidden lacre="{!!$carabina_gdl->lacre_entrada!!}" id="lacre_entrada_gdl" ></span>
            <span hidden estado_geral="{!!mb_strtolower($carabina_gdl->estado_geral)!!}" id="estado_geral_gdl" ></span>
            <span hidden marca="{!!mb_strtoupper($carabina_gdl->marca)!!}" id="marca_gdl" ></span>
            <span hidden status_serie="{!!mb_strtolower($carabina_gdl->status_serie)!!}" id="estado_serie_gdl" ></span>
            <span hidden num_serie="{!!mb_strtolower($carabina_gdl->num_serie)!!}" id="num_serie_gdl" ></span>
            <span hidden capacidade="{!!mb_strtolower($carabina_gdl->capacidade)!!}" id="capacidade_gdl" ></span>
            <span hidden acabamento="{!!mb_strtolower($carabina_gdl->acabamento)!!}" id="acabamento_gdl" ></span>
            <span hidden patrimonio="{!!mb_strtolower($carabina_gdl->patrimonio)!!}" id="patrimonio_gdl" ></span>
            <span hidden lacre_saida="{!!mb_strtolower($carabina_gdl->lacre_saida)!!}" id="lacre_saida_gdl" ></span>
            <span hidden fabricacao="{!!mb_strtolower($carabina_gdl->fabricacao)!!}" id="fabricacao_gdl" ></span>
            <span hidden modelo="{!!mb_strtolower($carabina_gdl->modelo)!!}" id="modelo_gdl" ></span>
            <span hidden calibreNominal="{!!mb_strtolower($carabina_gdl->calibre_nominal)!!}" id="calibre_gdl" ></span>
            <span hidden funcionamento="{!!mb_strtolower($carabina_gdl->funcionamento)!!}" id="funcionamento_gdl" ></span>
        @endforeach 
        @empty($armas)
            @else
